...
import type { SyncProgressModal } from '../components/SyncProgressModal';
  ...
  if (!plugin.statusBarItemEl) {
    ...
    plugin.statusBarItemEl.addEventListener('click', () => {
      if (!plugin.progressModal) {
        plugin.progressModal = new (require('../components/SyncProgressModal')
          .SyncProgressModal)(plugin.app, () => {
          plugin.progressModal = null as unknown as SyncProgressModal;
        });
      }
      if (plugin.progressModal) {
        plugin.progressModal.setProgress(plugin.processedNotes, plugin.totalNotes ?? undefined);
        plugin.progressModal.open();
      }
    });
    plugin.statusBarItemEl.setAttribute('aria-label', 'KeepSidian sync progress');
    plugin.statusBarItemEl.setAttribute('title', 'KeepSidian sync progress');
    ...
    if ((plugin.statusBarItemEl as any).appendChild) {
      (plugin.statusBarItemEl as any).appendChild(plugin.progressContainerEl);
    }
  } else {
...
export function reportSyncProgress(plugin: KeepSidianPlugin) {
  ...
  } else if (plugin.statusBarItemEl && (plugin.statusBarItemEl as any).setText) {
    (plugin.statusBarItemEl as any).setText(
      total ? `Sync: ${plugin.processedNotes}/${total}` : `Sync: ${plugin.processedNotes}`
    );
  }
  if (
...
export function setTotalNotes(plugin: KeepSidianPlugin, total: number) {
  ...
  if (plugin.statusTextEl) {
    ...
    (plugin.statusBarItemEl as any).setText(`Sync: ${plugin.processedNotes}/${total}`);
  }
  if (plugin.progressContainerEl) {
    plugin.progressContainerEl.classList.remove('indeterminate');
  }
