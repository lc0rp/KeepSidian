...
function setStatusBarTooltip(plugin: KeepSidianPlugin, tooltip: string) {
        if (plugin.statusBarItemEl && (plugin.statusBarItemEl as any).setAttribute) {
...
function ensureStatusBarElements(plugin: KeepSidianPlugin) {
        if (!plugin.statusBarItemEl) {
                ...
                plugin.statusBarItemEl.setAttribute("aria-label", "KeepSidian sync actions");
...
function showStatusMenu(plugin: KeepSidianPlugin, evt?: MouseEvent) {
        ...
        menu.addItem((item) => {
                item.setTitle("Two-way sync").setDisabled(syncing).onClick(() => {
                        plugin.performTwoWaySync();
                });
        });

        menu.addItem((item) => {
                item.setTitle("Import only").setDisabled(syncing).onClick(() => {
                        plugin.importNotes();
                });
        });

        menu.addItem((item) => {
                item.setTitle("Upload only").setDisabled(syncing).onClick(() => {
                        plugin.pushNotes();
                });
        });
