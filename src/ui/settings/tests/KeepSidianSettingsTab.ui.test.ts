...
jest.mock('obsidian', () => {
  ...
  class Setting {
    ...
    setDisabled(disabled: boolean) { if (disabled) this.settingEl.classList.add('is-disabled'); return this; }
    ...
    addText(cb: (text: any) => void) { 
      ...
      const text = {
        ...
        setValue: jest.fn((val: string) => { inputEl.value = val; return text; }),
        onChange: jest.fn((fn: (v: string)=>void) => { inputEl.addEventListener('input', () => fn(inputEl.value)); return text; })
      } as any;
      cb(text);
      return this;
    }
    ...
    addToggle(cb: (toggle: any) => void) {
      ...
      this.settingEl.appendChild(inputEl);
      const toggle = {
        setValue: jest.fn((v: boolean) => { inputEl.checked = v; return toggle; }),
        onChange: jest.fn((fn: (v: boolean)=>void) => { inputEl.addEventListener('change', () => fn(inputEl.checked)); return toggle; })
      } as any;
      cb(toggle);
      return this;
    }
    ...
    addButton(cb: (button: any) => void) {
      ...
      const button = {
        setButtonText: jest.fn((t: string) => { buttonEl.textContent = t; return button; }),
        setCta: jest.fn(() => button),
        onClick: jest.fn((fn: ()=>void) => { buttonEl.addEventListener('click', fn); return button; }),
      } as any;
      cb(button);
      return this;
    }
    ...
    addSlider(cb: (slider: any) => void) {
      ...
      const slider = {
        ...
        setValue: jest.fn((v: number) => { (inputEl as any).value = String(v); return slider; }),
        setDynamicTooltip: jest.fn(() => slider),
        onChange: jest.fn((fn: (v: number) => void) => { inputEl.addEventListener('input', () => fn(Number((inputEl as any).value))); return slider; })
      } as any;
      cb(slider);
      return this;
    }
...
describe('KeepSidianSettingsTab UI interactions', () => {
  ...
  const TEST_MANIFEST = {
    id: 'keepsidian', name: 'KeepSidian', author: 'lc0rp', version: '0.0.1', minAppVersion: '0.0.1', description: 'Import Google Keep notes.'
  };
  ...
  beforeEach(() => {
    ...
    plugin.subscriptionService = {
      ...
      getEmail: jest.fn(), getCache: jest.fn(), setCache: jest.fn(), fetchSubscriptionInfo: jest.fn(), checkSubscription: jest.fn(),
    } as any;
  ...
  test('token field show/hide toggle and onChange save', async () => {
    ...
    // Toggle show/hide
    const showBtn = Array.from(container.querySelectorAll('button')).find(b => b.textContent === 'Show') as HTMLButtonElement;
  ...
  test('retrieve token button calls flow with valid email and github open button exists', async () => {
    ...
    const retrieveBtn = Array.from(container.querySelectorAll('button')).find(b => b.textContent === 'Retrieval wizard') as HTMLButtonElement;
    expect(retrieveBtn).toBeTruthy();
    retrieveBtn.click();
    expect(initRetrieveToken).toHaveBeenCalled();

    // Also ensure the GitHub instructions button exists
    const githubBtn = Array.from(container.querySelectorAll('button')).find(b => b.textContent === 'Github KIM instructions');
  ...
  test('sync log path change persists', async () => {
    const container = tab.containerEl;
    await (tab as any).addAutoSyncSettings(container);
    const inputs = Array.from(container.querySelectorAll('input')) as HTMLInputElement[];
    // First input is checkbox from toggle; second one should be the sync log path text input
    const textInputs = inputs.filter(i => i.type !== 'checkbox');
    const logInput = textInputs[0];
    logInput.value = '_custom.log';
    logInput.dispatchEvent(new Event('input'));
    expect(plugin.settings.syncLogPath).toBe('_custom.log');
  });
  ...
  test('sync interval edits and triggers restart when enabled, gated for non-subscribers', async () => {
    ...
    const container = tab.containerEl;
    await (tab as any).addAutoSyncSettings(container);
    const textInputs = Array.from(container.querySelectorAll('input')).filter(i => i.type === 'text') as HTMLInputElement[];
    ...
    (container2 as any).createEl = (tag: string, opts?: any) => {
      const el = document.createElement(tag); if (opts?.text) el.textContent = opts.text; (el as any).createEl = (container as any).createEl; container2.appendChild(el); return el;
    };
    ...
    const disabledSetting = container2.querySelector('.requires-subscription');
